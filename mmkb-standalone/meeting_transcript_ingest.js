...
async function ingestTranscript({ filePath, title, link, acl, overrideKey, maxSeconds, maxWords } = {}) {
  ...
  if (AUDIO_EXT.has(ext)) {
    ...
    // Deliberately does NOT echo the filename/title into `text` (only `source`
    // carries that) -- this is a failure placeholder, not real content, and a
    // filename like "Supply Chain end to end visibility...mp4" re-embedded
    // here would lexically outrank the actually-relevant file on any query
    // that happens to share words with that filename (see searchSync's lex
    // gate in engine.js: it matches on raw text tokens, not "is this real").
    else return [{ id: name, source: `Meeting: ${docTitle}`, link: baseLink, acl: baseAcl,
      text: `(this file has no transcript indexed — no audio-transcription-capable model key is configured, or the call to the configured provider failed)`,
