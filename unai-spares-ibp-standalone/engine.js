...
const USE_CASES = {
  ...
  spares_planning_ibp: {
    name: "Spares Planning Process (IBP)",
    description: "Data-center spares planning: install-base failure → intermittent demand → returns/refurbish → multi-echelon target → net requirements → replenishment & execution. Mirrors the Bristlecone IBP overlay — but one UNAI runs the whole pipeline across SAP IBP, S/4HANA, ServiceNow and the BQML feature store.",
  ...
  rma_execution: {
    name: "RMA Execution Process",
    description: "Return-merchandise authorization end-to-end: intake → claim/terms → inspection → triage → parametric test → failure analysis → root cause → repair-vs-replace → work order → rework/calibration → final test → disposition → CoC → close the loop to planning. 15 specialist agents in the overlay; one UNAI here, mixing ML/VLM/LLM/OPT/rules behind shared evidence + governance.",
    gen1Agents: ["RMA Intake Agent", "Claims & Terms Agent", "Visual Inspection Agent", "Triage Agent",
