...
async function callAnthropic(messages, p, opts, signal) {
  ...
  const body = { model: p.model, max_tokens: opts.max_tokens || 500, temperature: opts.temperature ?? 0.2,
...
async function chatWithFailover(messages, opts = {}) {
  ...
  const tierModel = opts.tier ? TIER_MODEL()[opts.tier] : null;   // complexity-based override
  const tried = [];
  for (const p0 of providers) {
    ...
    try {
      ...
      const res = await fetch(p.base.replace(/\/$/, "") + "/chat/completions", {
        ...
        body: JSON.stringify({ model: p.model, temperature: opts.temperature ?? 0.2, max_tokens: opts.max_tokens || 500, messages }),
      });
...
async function narrateOne(ev, cfg, timeoutMs = 8000, retriesLeft = 2) {
  ...
  try {
    ...
    const u = data?.usage || {};
    return text ? { text: text.trim(), tokensIn: u.prompt_tokens || 0, tokensOut: u.completion_tokens || 0 } : null;
...
async function enrichEvidence(evidence, overrideKey) {
  ...
  let ok = 0, tokensIn = 0, tokensOut = 0;
  const CONCURRENCY = 3, STAGGER_MS = 400;
  // SCOPED, OPT-IN ONLY: some corporate networks intercept HTTPS to specific
  // providers (seen here with api.mistral.ai) with a self-signed cert, which
  // Node rejects by default (SELF_SIGNED_CERT_IN_CHAIN) -- a real network
  // condition, not a bug in this code. Set ALLOW_INSECURE_TLS=<provider label,
  // e.g. "mistral"> to skip cert verification ONLY for the duration of this
  // call, ONLY when that exact provider is active. Restored immediately after,
  // even on error. Not something to leave set outside a diagnosed network like
  // this one -- it trusts any certificate presented for that connection.
  const insecureFor = (process.env.ALLOW_INSECURE_TLS || "").split(",").map(s => s.trim().toLowerCase()).filter(Boolean);
  const needsInsecure = insecureFor.includes(cfg.label);
  ...
  try {
    for (let i = 0; i < evidence.length; i += CONCURRENCY) {
      ...
      await Promise.all(batch.map(async (ev, j) => {
        ...
        if (r) { ev.llmExplanation = r.text; ev.explanationSource = cfg.label;
...
module.exports = { enrichEvidence, narrateOne, resolveConfig, resolveProviders, chatWithFailover, answerGrounded, nlToSql, LLM_REGISTRY, PRESETS };
