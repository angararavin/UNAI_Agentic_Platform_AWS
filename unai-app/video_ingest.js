  ...
  try {
    ...
    if (!mergedChunks.length) {
      return [{ id: name, source: `Video: ${docTitle}`, link: baseLink, acl: baseAcl,
        text: `(video "${path.basename(filePath)}" produced no transcribable audio and no describable frames — no ASR/vision key configured, or the video has neither speech nor legible on-screen content)`,
