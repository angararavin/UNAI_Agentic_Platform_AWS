...
// server/workflow/llm-client.ts
var llm_client_exports = {};
__export(llm_client_exports, {
  PRIMARY_GEMINI_MODEL: () => PRIMARY_GEMINI_MODEL,
  callLLM: () => callLLM,
  cleanJson: () => cleanJson,
  getLLMClient: () => getLLMClient,
  isCustomApiConfigured: () => isCustomApiConfigured,
  isGeminiConfigured: () => isGeminiConfigured
});
function isCustomApiConfigured() {
  const key = process.env.CUSTOM_AI_API_KEY;
  return Boolean(
    key && key.trim() !== "" && key !== "your-custom-api-key" && key !== "dummy-key"
  );
}
...
function getLLMClient(preferredProvider) {
  ...
  if (provider === "custom_api" && !hasCustomKey && hasGemini) {
    ...
    const geminiClient2 = new import_genai.GoogleGenAI(geminiKey2 ? { apiKey: geminiKey2 } : {});
    return {
      provider: "gemini",
      modelName: PRIMARY_GEMINI_MODEL,
      client: geminiClient2,
      type: "gemini"
    };
  }
  if (provider === "ollama") {
    ...
    const client = new import_ollama.ChatOllama({
      baseUrl,
      model,
      temperature: 0.1
    });
    return { provider: "ollama", modelName: model, client, type: "langchain" };
  }
  if (provider === "custom_api") {
    ...
    const client = new import_openai.ChatOpenAI({
      apiKey,
      configuration: { baseURL },
      modelName,
      temperature: 0.1
    });
    return { provider: "custom_api", modelName, client, type: "langchain" };
  }
  const geminiKey = process.env.GEMINI_API_KEY;
  const geminiClient = new import_genai.GoogleGenAI(geminiKey ? { apiKey: geminiKey } : {});
  return { provider: "gemini", modelName: PRIMARY_GEMINI_MODEL, client: geminiClient, type: "gemini" };
}
...
async function invokeGeminiFast(systemPrompt, userPrompt) {
  const gemini = new import_genai.GoogleGenAI({ apiKey: process.env.GEMINI_API_KEY });
  try {
    const response = await withTimeout(
...
var import_ollama, import_openai, import_genai, PRIMARY_GEMINI_MODEL;
var init_llm_client = __esm({
  "server/workflow/llm-client.ts"() {
    import_ollama = require("@langchain/ollama");
...
var require_tokenCompare = __commonJS({
  "agents/unai/tokenCompare.cjs"(exports2, module2) {
    ...
    var { narrateReal, resolveConfig } = require_liveNarration();
...
var require_unaiAdapter = __commonJS({
  "agents/unai/unaiAdapter.cjs"(exports2, module2) {
    ...
    async function demandAnalystNode2(state) {
    ...
    async function supplyImpactAnalystNode2(state) {
      const startTime = Date.now();
      const { item, surgeInput, demandAnalysis } = state;
      const effectiveInventory = (surgeInput.overrideInventory !== void 0 ? surgeInput.overrideInventory : item.currentInventory) - item.reservedInventory;
      ...
      const plantDailyCapacity = surgeInput.overrideDailyCapacity !== void 0 ? surgeInput.overrideDailyCapacity : item.productionCapacity.dailyCapacityUnits;
    ...
    async function plannerRecommendationNode2(state) {
      const startTime = Date.now();
    ...
    module2.exports = { demandAnalystNode: demandAnalystNode2, supplyImpactAnalystNode: supplyImpactAnalystNode2, plannerRecommendationNode: plannerRecommendationNode2 };
  }
});
...
// server/dummy-data.ts
var INITIAL_SUPPLY_ITEMS = [
  {
    sku: "SKU-1049",
...
// server/agents/hitl-agent.ts
function humanReviewInitNode(state) {
  const startTime = Date.now();
  ...
  const defaultSelectedActions = actionMatrix.map((a) => a.id);
  const humanReview = {
...
function processHumanDecision(runResult, decision, supplyItemsCatalog) {
  const startTime = Date.now();
  ...
  const erpOrdersCreated = [];
  const supplierNoticesSent = [];
  ...
  const decisionLog = {
    ...
    summary: status === "rejected" ? `Human planner (${reviewedBy}) rejected proposed mitigation strategy. Rationale: ${plannerNotes || "Plan rejected."}` : `Human planner (${reviewedBy}) executed decision: ${status.toUpperCase()}. Authorized ${approvedActionIds.length}/${actions.length} mitigations ($${totalAuthorizedCostUsd.toLocaleString()} approved budget). Downstream dispatch completed.`,
    keyOutputs: {
      finalStatus: status,
      authorizedCostUsd: totalAuthorizedCostUsd,
...
var unaiAdapter = require_unaiAdapter();
var demandAnalystNode = unaiAdapter.demandAnalystNode;
var supplyImpactAnalystNode = unaiAdapter.supplyImpactAnalystNode;
var plannerRecommendationNode = unaiAdapter.plannerRecommendationNode;

// server/workflow/graph.ts
var OrderSurgeStateAnnotation = import_langgraph.Annotation.Root({
  item: import_langgraph.Annotation,
  surgeInput: import_langgraph.Annotation,
  provider: import_langgraph.Annotation,
  modelName: import_langgraph.Annotation,
  demandAnalysis: import_langgraph.Annotation,
  supplyAnalysis: import_langgraph.Annotation,
  plannerRecommendations: import_langgraph.Annotation,
  humanReview: import_langgraph.Annotation,
  logs: (0, import_langgraph.Annotation)({
    reducer: (curr, update) => curr.concat(update),
    default: () => []
  })
});
function createOrderSurgeGraph() {
  const workflow = new import_langgraph.StateGraph(OrderSurgeStateAnnotation).addNode("demand_analyst", demandAnalystNode).addNode("supply_impact_analyst", supplyImpactAnalystNode).addNode("planner_recommendation", plannerRecommendationNode).addNode("human_review_gate", humanReviewInitNode).addEdge(import_langgraph.START, "demand_analyst").addEdge("demand_analyst", "supply_impact_analyst").addEdge("supply_impact_analyst", "planner_recommendation").addEdge("planner_recommendation", "human_review_gate").addEdge("human_review_gate", import_langgraph.END);
  return workflow.compile();
}
async function runOrderSurgeAnalysis(item, surgeInput, provider) {
  ...
  const perNode = [
    finalState.demandAnalysis?.unaiTokenComparison,
    finalState.supplyAnalysis?.unaiTokenComparison,
    finalState.plannerRecommendations?.unaiTokenComparison
  ].filter(Boolean);
  let unaiTokenComparison = void 0;
  if (perNode.length) {
    const sum = (key, path3) => perNode.reduce((a, tc) => {
      let v = tc;
      for (const p of path3) v = v && v[p];
    ...
    unaiTokenComparison = {
      ...
      perLayerTokens: (() => {
        const withLayers = perNode.filter((tc) => Array.isArray(tc.perLayerTokens));
        if (!withLayers.length) return null;
        const byId = {};
        withLayers.forEach((tc) => {
          tc.perLayerTokens.forEach((l) => {
      ...
      perLayerMeasured: perNode.find((tc) => tc.perLayerMeasured)?.perLayerMeasured || null
    };
  }
  return {
...
var supplyItems = JSON.parse(JSON.stringify(INITIAL_SUPPLY_ITEMS));
var analysisHistory = [];
function determineInitialProvider() {
  const customKey = process.env.CUSTOM_AI_API_KEY;
  const hasCustomKey = Boolean(
    customKey && customKey.trim() !== "" && customKey !== "your-custom-api-key" && customKey !== "dummy-key"
  );
...
var sessionPreferredProvider = determineInitialProvider();
...
app.post("/api/config/test", async (req, res) => {
  ...
  try {
    if (targetProvider === "ollama") {
      ...
      const response = await fetch(pingUrl, { signal: AbortSignal.timeout(4e3) });
      if (!response.ok) {
    ...
    const client = getLLMClient2("gemini");
    ...
    return res.json({
      ...
      guidance: targetProvider === "ollama" ? "Ensure Ollama is running (`ollama serve` and `ollama run llama3.2`). If running in a Docker container or remote server, ensure the OLLAMA_BASE_URL is reachable." : targetProvider === "custom_api" ? "Verify CUSTOM_AI_API_BASE_URL and CUSTOM_AI_API_KEY in .env. The endpoint must support OpenAI-compatible chat completions." : "Check GEMINI_API_KEY in the environment or secrets panel."
    });
  }
});
...
app.get("/api/supply-chain/items", (req, res) => {
  res.json(supplyItems);
});
app.get("/api/supply-chain/items/:sku", (req, res) => {
...
app.post("/api/supply-chain/items", (req, res) => {
...
app.post("/api/analyze-surge", async (req, res) => {
  try {
    const surgeInput = req.body;
...
app.post("/api/analysis/:id/human-decision", (req, res) => {
  const { id } = req.params;
