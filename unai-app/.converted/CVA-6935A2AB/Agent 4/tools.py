"""
Agent 04: PO Aging Intelligence Tools
Deterministic calculation engine for aged purchase order diagnosis and demand validation.
"""
    ...
    def diagnose_po_root_cause(
        ...
        if rec_q > 0 and rem_q > 0:
            ...
            return {
                ...
                "rationale": f"Partial goods receipt received ({rec_q} units). Open residual of {rem_q} units requires vendor status check.",
                "priority": "MEDIUM",
                "consequential_action_required": True
            }

        # 7. Default Valid Open
        return {
            "root_cause": "VALID_FUTURE_DEMAND",
            "recommended_action": "KEEP_OPEN",
            "rationale": "Purchase order is validly open, pending delivery, and mapped to upcoming demand.",
            "priority": "LOW",
            "consequential_action_required": False
        }


class LocalAgingDataLoader:
    """Loads CSV files locally from PO_Aging_Intelligence_Agent_Synthetic_Data directory."""

    def __init__(self, data_dir: Optional[str] = None):
        if data_dir is None:
            base = os.path.dirname(os.path.abspath(__file__))
            data_dir = os.path.join(base, "PO_Aging_Intelligence_Agent_Synthetic_Data")
        self.data_dir = data_dir

    def load_demo_cases(self) -> pd.DataFrame:
        return pd.read_csv(os.path.join(self.data_dir, "demo_cases.csv"))

    def load_open_pos(self) -> pd.DataFrame:
        return pd.read_csv(os.path.join(self.data_dir, "current_open_purchase_orders.csv"))

    def load_requirements(self) -> pd.DataFrame:
        return pd.read_csv(os.path.join(self.data_dir, "requirements.csv"))

    def load_release_status(self) -> pd.DataFrame:
        return pd.read_csv(os.path.join(self.data_dir, "po_release_status.csv"))

    def load_goods_receipts(self) -> pd.DataFrame:
        return pd.read_csv(os.path.join(self.data_dir, "goods_receipts.csv"))

    def load_invoice_status(self) -> pd.DataFrame:
        return pd.read_csv(os.path.join(self.data_dir, "invoice_status.csv"))

    def load_change_history(self) -> pd.DataFrame:
        return pd.read_csv(os.path.join(self.data_dir, "po_change_history.csv"))
