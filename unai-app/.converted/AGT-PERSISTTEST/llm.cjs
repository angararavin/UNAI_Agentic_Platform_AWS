...
// Native UNAI AI Gateway: per-provider key pools, rotation, cooldown,
// load-balancing, budget + spend telemetry. llm.js provides the transports;
// gateway.js decides which provider+key to use and records the outcome.
const gw = require("./gateway.cjs");
...
function resolveProviders() {
  ...
  gw.routerOrder().forEach(n0 => {   // env LLM_ROUTER_ORDER + any UI-configured provider
    const n = (n0 === "claude") ? "anthropic" : n0;   // friendly alias
    let p;
    if (LLM_REGISTRY[n]) { const keys = gw.envKeysFor(n); const k = keys[0] || process.env[LLM_REGISTRY[n].env];   // plural <P>_API_KEYS or single <P>_API_KEY
      ...
      const ck = process.env[U + "_KEYS"] || process.env[U + "_KEY"];
      if (process.env[U + "_BASE"] && ck) p = { base: process.env[U + "_BASE"], key: String(ck).split(/[,\s]+/).filter(Boolean)[0], model: process.env[U + "_MODEL"] || "default", label: n, kind: (process.env[U + "_KIND"] || "openai").toLowerCase() }; }
    if (p) list.push(p);
  });
  const seen = new Set();
...
async function callAnthropic(messages, p, opts, signal) {
  ...
  try { require("./containment.cjs").assertEgress(p.base, "llm:anthropic"); } catch (e) { if (e.code === "EGRESS_BLOCKED") throw e; }
...
async function chatWithFailover(messages, opts = {}) {
  ...
  const tierModel = tier ? TIER_MODEL()[tier] : null;   // complexity-based model override
  // Expand each provider into its key pool (plural env keys, else the single key),
  // then let the gateway ORDER the attempts (tier preference + load-balance +
  // budget + health). Backward compatible: one key + default policy == old behaviour.
  const candidates = providers.map(p => { const keys = gw.envKeysFor(p.label); const ks = keys.length ? keys : [p.key]; gw.pool(p.label, ks); return { ...p, keys: ks }; });
  const order = gw.routeOrder(candidates, tier);
  const tried = [];
  for (const p0 of order) {
    // A tier model override only applies when the model family matches the
    // provider family (a Claude id must not hit an OpenAI-shape endpoint).
    const isClaudeTier = tierModel ? /claude/i.test(tierModel) : false;
    const useTier = !!tierModel && ((p0.kind === "anthropic") === isClaudeTier);
    const model = useTier ? tierModel : p0.model;
    // Try each rotating key of THIS provider (cooling one → next key) before
    // moving to the next provider.
    const keyAttempts = Math.max(1, (p0.keys || []).length);
    for (let ki = 0; ki < keyAttempts; ki++) {
      const ke = gw.pickKey(p0.label, p0.keys);
      if (!ke) { tried.push(p0.label + ":all-keys-cooling"); break; }
      const p = { ...p0, key: ke.key, model };
      const kt = ke.key.slice(-4); const t0 = Date.now();
      const done = (ok, status, tk) => { gw.reportResult(p.label, ke.key, { ok, status, model, tokensIn: tk && tk.tokensIn, tokensOut: tk && tk.tokensOut });
        gw.recordAttempt({ provider: p.label, keyTail: kt, model, tier, ok, status: ok ? 200 : status, tokensIn: tk && tk.tokensIn, tokensOut: tk && tk.tokensOut, costUsd: ok ? gw.costUsd(model, tk && tk.tokensIn, tk && tk.tokensOut) : 0, ms: Date.now() - t0 }); };
      ...
      try {
        if (p.kind === "anthropic") {
          ...
          if (a.retry) { tried.push(p.label + ":" + (a.status || "err")); done(false, a.status || "err"); continue; }
          if (a.content) { done(true, 200, a); return { content: a.content.trim(), provider: p.label, model, keyTail: kt, tried,
            tokensIn: a.tokensIn, tokensOut: a.tokensOut, cacheRead: a.cacheRead, cacheWrite: a.cacheWrite, cached: a.cached }; }
          tried.push(p.label + ":empty"); done(false, "empty"); continue;
        }
        ...
        try { require("./containment.cjs").assertEgress(p.base, "llm:openai"); } catch (e) { if (e.code === "EGRESS_BLOCKED") throw e; }
        ...
        if (res.status === 429 || res.status >= 500) { tried.push(p.label + ":" + res.status); done(false, res.status); continue; }   // quota/rate/server → cool key, next key/provider
        if (!res.ok) { tried.push(p.label + ":" + res.status); done(false, res.status); continue; }
        const data = await res.json();
        const content = data && data.choices && data.choices[0] && data.choices[0].message && data.choices[0].message.content;
        if (content) { const det = data.usage && data.usage.prompt_tokens_details;
          const tokensIn = data.usage && data.usage.prompt_tokens, tokensOut = data.usage && data.usage.completion_tokens;
          done(true, 200, { tokensIn, tokensOut });
          return { content: content.trim(), provider: p.label, model, keyTail: kt, tried, tokensIn, tokensOut,
            cacheRead: (det && det.cached_tokens) || 0, cacheWrite: 0, cached: !!(det && det.cached_tokens) }; }
        tried.push(p.label + ":empty"); done(false, "empty");
      } catch (_) { tried.push(p.label + ":timeout"); done(false, "timeout"); } finally { clearTimeout(timer); }
    }
  }
...
async function narrateOne(ev, cfg, timeoutMs = 8000, retriesLeft = 2) {
  ...
  try {
    let res;
    try {
      require("./containment.cjs").assertEgress(cfg.base, "llm:grounded");
      res = await fetch(cfg.base.replace(/\/$/, "") + "/chat/completions", {
        method: "POST",
        headers: { "Content-Type": "application/json", "Authorization": `Bearer ${cfg.key}` },
        signal: ctrl.signal,
        // 80 was too tight for a "thinking" model (e.g. gemini-3.6-flash):
        // it spends part of the budget on invisible reasoning tokens before
        // any visible content, so a low cap can leave `message.content`
        // empty or truncated mid-thought even on a successful call (a real
        // request measured ~140 total tokens for a one-word reply). 500
        // matches chatWithFailover's own real-call default elsewhere in
        // this file, giving reasoning room without still risking an empty
        // "connection test" result on providers/models that think first.
...
// ---- ROADMAP: async Batch API (offline/bulk jobs at ~50% provider discount) ----
// For NON-interactive work only — ingesting a 1000-slide deck, backfilling video
// transcripts, embedding a whole library. Submit many requests as one batch to the
// provider's Batch endpoint (OpenAI POST /v1/batches, Anthropic Message Batches),
// which runs async (minutes–24h) at roughly half the price. The interactive Q&A
// path stays on chatJSON/answerGrounded — batching is not for live requests.
// Status: ROADMAP — shape is below; wire to the provider + a small job store.
async function batchProcess(requests, opts = {}) {
  throw new Error("batchProcess: ROADMAP — not yet implemented. Plan: (1) resolveConfig() for the gateway; " +
    "(2) POST requests to the provider batch endpoint (OpenAI /v1/batches or Anthropic /v1/messages/batches); " +
    "(3) persist the returned batch id; (4) poll until complete; (5) return results. Use for bulk " +
    "ingestion/backfill only — interactive answers stay on chatJSON/answerGrounded.");
}

module.exports = { enrichEvidence, narrateOne, resolveConfig, resolveProviders, chatWithFailover, chatJSON, safeJsonParse, answerGrounded, nlToSql, batchProcess, LLM_REGISTRY, PRESETS, pickTier, compressContext,
  // Native AI Gateway surface (multi-key pools, routing, budget, telemetry)
  gatewayStats: gw.gatewayStats, gatewayResetStats: gw.resetStats, gateway: gw };
