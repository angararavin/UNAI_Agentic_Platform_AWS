...
// Real (not modeled) per-layer token measurement: BPE-tokenize the ACTUAL
// data that passes through each layer, same tokenizer tokenCompare.cjs uses
// for the original-app side, so every layer's number is a real measurement
// of real content — never a fixed per-call rate.
let _encode = null;
try { _encode = require("gpt-tokenizer").encode; } catch (_e) { /* browser bundle without the dep — realTok falls back below */ }
const realTok = (v) => { const s = typeof v === "string" ? v : JSON.stringify(v ?? ""); return _encode ? _encode(s).length : Math.ceil(s.length / 4); };
...
class Layers {
  constructor(systems, mapper, memory, metrics, trace) {
    ...
    // ---- REAL per-layer token measurement (BPE-tokenized actual data, not a modeled rate) ----
    this.realTokens = { Perception:{in:0,out:0}, Memory:{in:0,out:0}, Reasoning:{in:0,out:0},
      Evidence:{in:0,out:0}, Action:{in:0,out:0}, Collaboration:{in:0,out:0}, Explainability:{in:0,out:0} };
    this.explainEntriesReal = [];   // one entry per explain() call, order-matched to evidenceLog — lets a
                                     // caller upgrade any entry to a real LIVE LLM measurement afterwards
  }
  _realAdd(layer, inTok, outTok) { const r = this.realTokens[layer]; r.in += inTok; r.out += outTok; }
  ...
  perceive(system, entity, filter) {
    ...
    if (this.cache[key]) {   // shared cognition: reuse, no re-read / re-map / re-context
      ...
      this._realAdd("Perception", realTok(key), realTok(this.cache[key]));  // real: tiny cache-key lookup in, full real data out
  ...
  recall(key) { const t=NOW(); const v=this.memory[key]; this._hit("Memory", `recall “${key}”`, NOW()-t); this.trace("Memory", `recall ${key}`);
    this._realAdd("Memory", 0, realTok(v)); return v; }
  ...
  explain(ev) {
    ...
    const promptText = "You are the Explainability layer of an autonomous supply-chain agent. " +
...
const TOOL_PACKS = {
  ...
  recoverable_units(L) {
    ...
    const newSupplyCostUsd  = row.new_supply_cost_usd  != null ? row.new_supply_cost_usd  : 9000;
    ...
    const ev = L.evidence(`Recoverable units: ${recoverableUnits} of ${fieldReturnsCount} field returns (${recoveryRatePct}% recovery rate)`,
      [{ name: "recovery rate applied", weight: 0.20 }, { name: "repair vs new-build cost delta", weight: 0.12 }]);
    L.explain(ev);
  ...
  repair_cost_total(L) {
    const c = L.recall("refurb_repair_calc") || { recoverableUnits: 400, repairCostTotal: 480000 };
  ...
  new_supply_cost_total(L) {
    const c = L.recall("refurb_repair_calc") || { newSupplyCostTotal: 3600000 };
  ...
  net_savings_usd(L) {
    const c = L.recall("refurb_repair_calc") || { netSavingsUsd: 3120000 };
  ...
  forecast_data(L) {
    ...
    const salesOverrideUnits     = row.sales_override_units     != null ? row.sales_override_units     : 12000;
    const marketingOverrideUnits = row.marketing_override_units != null ? row.marketing_override_units : 6000;
    ...
    const ev = L.evidence(`26-week consensus forecast: ${totalOverride.toLocaleString()} override units distributed across the horizon`,
  ...
  chart_data(L) {
    ...
    const signalVolumeUnits = row.signal_volume_units != null ? row.signal_volume_units : 8000;
    const baseAvg = 1200;
    const chartData = [];
    for (let i = 1; i <= 13; i++) {
      const statisticalBaseline = Math.round(baseAvg + Math.sin(i / 2) * 100);
      let rawSignal = 0;
      if (i >= 2 && i <= 5) { const weight = i === 2 ? 0.15 : i === 3 ? 0.45 : i === 4 ? 0.30 : 0.10; rawSignal = Math.round(signalVolumeUnits * weight); }
    ...
    const ev = L.evidence(`Demand sensing: ${anomalyDetected ? "anomaly confirmed" : "regular wave"} — ${signalVolumeUnits.toLocaleString()}-unit signal at +${signalSpikePct}%`,
      [{ name: "signal spike intensity", weight: 0.20 }, { name: "cluster buildout schedule", weight: 0.10 }]);
    L.explain(ev);
  ...
  champion_model(L) {
    ...
    const ev = L.evidence(`Champion model selected: ${championModel} (adoption: ${marketAdoptionRate})`,
      [{ name: "MAPE accuracy comparison", weight: 0.20 }, { name: "market adoption multiplier", weight: 0.10 }]);
    L.explain(ev);
  ...
  current_kpi_value(L) {
    ...
    const ev = L.evidence(`Orchestrating ${coordinatedAgents.length}-agent mitigation for ${kpiBreachType} in ${affectedRegion} (SKU ${impactedSku})`,
      [{ name: "cross-agent routing", weight: 0.18 }, { name: "buffer inventory availability", weight: 0.10 }]);
    L.explain(ev);
  ...
  solvability_recommendation(L) {
    ...
    const diffDays = Math.ceil(Math.abs(now.getTime() - buyDate.getTime()) / 86400000);
    ...
    if (/dust|hot|fan|temp|throttle|cooling|overheat|airflow|heatsink/i.test(text)) { category = "thermal/airflow"; failureRateAfrPct = 1.4; mtbfHours = 36200; repairabilityRatePct = 98.6; firstTimeFixRatePct = 97.4; avgRepairTatDays = 0.2; salvageSavingsUsd = 28500; }
    else if (/power|controller|mosfet|post|amber|vrm|volt|rail|fuse|capacitor/i.test(text)) { category = "power/VRM"; failureRateAfrPct = 4.8; mtbfHours = 11400; repairabilityRatePct = 88.5; firstTimeFixRatePct = 89.1; avgRepairTatDays = 2.5; salvageSavingsUsd = 32000; }
    else if (/hbm|memory|ecc|substrate|corruption|micro-bump|bump|silicon|bga|interposer/i.test(text)) { category = "HBM/memory"; failureRateAfrPct = 3.2; mtbfHours = 16800; repairabilityRatePct = 81.2; firstTimeFixRatePct = 84.5; avgRepairTatDays = 4.2; salvageSavingsUsd = 41500; }
    else if (/coolant|liquid|manifold|seepage|leak|dielectric|cold plate/i.test(text)) { category = "liquid cooling"; failureRateAfrPct = 1.1; mtbfHours = 42000; repairabilityRatePct = 96.8; firstTimeFixRatePct = 95.4; avgRepairTatDays = 0.3; salvageSavingsUsd = 36000; }
    else if (/nvlink|retimer|lane|dropout|desync|switch tray|fabric/i.test(text)) { category = "NVLink/retimer"; failureRateAfrPct = 2.3; mtbfHours = 22800; repairabilityRatePct = 94.1; firstTimeFixRatePct = 93.0; avgRepairTatDays = 0.5; salvageSavingsUsd = 29800; }
    else { category = "general signal integrity"; failureRateAfrPct = isSolvable ? 2.1 : 3.9; mtbfHours = isSolvable ? 24500 : 15800; repairabilityRatePct = isSolvable ? 93.5 : 84.0; firstTimeFixRatePct = isSolvable ? 92.0 : 86.8; avgRepairTatDays = isSolvable ? 0.4 : 3.0; salvageSavingsUsd = isSolvable ? 19500 : 25800; }
    const solvabilityRecommendation = isSolvable
      ? `Perform a clean enterprise driver purge and reseat the unit in its PCIe slot; update cooling profiles before authorizing hardware RMA.`
      : `Physical hardware degradation identified (S/N: ${serialNumber}). Authorize factory RMA intake for certified diagnostic and repair.`;
    const failureRepairAnalysis = { failureRateAfrPct, mtbfHours, repairabilityRatePct, firstTimeFixRatePct, avgRepairTatDays, salvageSavingsUsd, category,
    ...
    const ev = L.evidence(`RMA disposition: ${disposition} (${warrantyStatus})`, [{ name: "defect signature match", weight: 0.20 }, { name: "warranty duration check", weight: 0.12 }]);
    L.explain(ev);
  ...
  disposition(L) {
    ...
    const ev = L.evidence(`Disposition: ${c.disposition}`, [{ name: "defect signature match", weight: 0.20 }]);
    L.explain(ev);
...
class UNAI {
  ...
  run(useCaseKey, injectedSystems) {
    ...
    const gen2LayerImpls = layersShared + capabilities;     // 7 shared + N tool packs
    ...
    // ---- REAL per-layer breakdown: BPE-tokenized actual data per layer (not
    // the modeled rate table above). Explainability here is still the LOCAL
    // measurement (real tokenization of the prompt/response text, no network
    // call) — a caller can upgrade it to a real LIVE LLM measurement per
    // decision using explainEntriesReal, see unaiAdapter.cjs.
    ...
    const observability = {
      ...
      layerTokensReal,                               // REAL per-layer breakdown (BPE-tokenized actual data, not modeled rates)
      explainEntriesReal,                            // per-decision entries for layerTokensReal.Explainability — upgradeable to a live LLM measurement
