...
"""
        ...
        # excess_obsolete is this domain's real UNAI capability (flagging
        # units that no longer need to be on order/on hand -- the aged,
        # possibly-no-longer-needed PO this agent investigates), systems
        # ["ANALYTICS"] lifted verbatim from engine.js's own pre-built use
        # case.
        try:
            ev, tc = run_unai(
                capability="excess_obsolete", systems=["ANALYTICS"], agent_id="agent_04",
                system_instruction=system_prompt, prompt=prompt, completion_text=fallback_reasoning,
            )
            reasoning = f"{ev.get('explanation') or fallback_reasoning} (this run — PO {po_rec.get('po_number')}, case {po_rec.get('case_id')}.)"
            confidence = ev.get("confidence", 0.97)
        except Exception:
