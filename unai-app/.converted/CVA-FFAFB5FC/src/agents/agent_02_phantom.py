...
"""
        ...
        # materials_availability is this domain's real UNAI capability
        # (components below safety stock -- the ERP-vs-usable reconciliation
        # this agent performs), systems ["SAP_MM"] lifted verbatim from
        # engine.js's own pre-built use case.
        try:
            ev, tc = run_unai(
                capability="materials_availability", systems=["SAP_MM"], agent_id="agent_02",
                system_instruction=system_prompt, prompt=prompt, completion_text=fallback_reasoning,
            )
            reasoning = f"{ev.get('explanation') or fallback_reasoning} (this run — material {rec.get('material_id')}, plant {rec.get('plant_id')}.)"
