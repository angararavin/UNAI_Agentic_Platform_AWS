...
"""
        ...
        # net_requirements is this domain's real UNAI capability (MRP
        # netting -- exactly the "clean net MRP inputs" this agent resolves
        # conflicting requirements toward), systems ["SAP_IBP"] lifted
        # verbatim from engine.js's own pre-built use case.
        try:
            ev, tc = run_unai(
                capability="net_requirements", systems=["SAP_IBP"], agent_id="agent_05",
                system_instruction=system_prompt, prompt=prompt, completion_text=fallback_reasoning,
            )
            reasoning = f"{ev.get('explanation') or fallback_reasoning} (this run — {case_rec.get('material_id')} at {case_rec.get('plant_id')}, case {case_rec.get('case_id')}.)"
            confidence = ev.get("confidence", 0.98)
        except Exception:
