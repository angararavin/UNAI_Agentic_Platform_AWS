...
"""
        ...
        # src/agents/unai_bridge.py + agents/unai/bridge.cjs). eta_prediction
        # is this domain's real, already-existing UNAI capability (delivery
        # ETA / drift prediction), systems ["ANALYTICS"] lifted verbatim from
        # engine.js's own pre-built Logistics & Transportation use case --
        # not guessed.
        try:
            ev, tc = run_unai(
                capability="eta_prediction", systems=["ANALYTICS"], agent_id="agent_01",
                system_instruction=system_prompt, prompt=prompt, completion_text=fallback_reasoning,
            )
            reasoning = f"{ev.get('explanation') or fallback_reasoning} (this run — PO {po_rec.get('po_number')}, supplier {supplier_stats.get('supplier_id')}.)"
            confidence = ev.get("confidence", 0.95 if not supplier_stats.get("is_sparse_data") else 0.65)
        except Exception:
            reasoning = fallback_reasoning
            confidence = 0.95 if not supplier_stats.get("is_sparse_data") else 0.65
            tc = None
