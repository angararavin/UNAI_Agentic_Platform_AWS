...
class Layers {
  ...
  evidence(decision, drivers, integrity) {
    ...
    const ev = {
      ...
      counterfactual: confidence >= TH
        ? `If confidence fell below ${THpct}% (e.g. the top driver weakened), this would route to a human approver instead of executing.`
        : `Reaching ${THpct}% confidence (e.g. stronger grounding data) would let this execute autonomously.`,
    };
  ...
  _detailExplain(ev, top, tail) {
    ...
    const outcome = ev.autonomous ? "Executed autonomously"
      : (ev.hold ? "Held — signal integrity" : "Routed to a human approver");
