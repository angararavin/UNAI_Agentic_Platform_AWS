...
(function () {
  ...
  window.fetch = function () {
    ...
    return origFetch.apply(this, args).then(function (res) {
      try {
        // This app's own fetch() calls pass a `URL` OBJECT, not a plain
        // string or a Request -- args[0].url doesn't exist on a URL
        // instance (that's Request's shape), so fall through to .href /
        // String(...) too. Its single kickoff endpoint is /api/pipeline.
        var raw = args[0];
        var url = (raw && typeof raw.url === "string") ? raw.url : (raw && typeof raw.href === "string") ? raw.href : raw;
        if (typeof url === "string" && /\/api\/pipeline/i.test(url)) {
