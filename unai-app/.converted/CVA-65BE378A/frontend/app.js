document.addEventListener('DOMContentLoaded', () => {
    ...
    function renderDashboard() {
        ...
        updateSelectedOptionResults(options, fin, isHealthy, data.commercialImpact, rec);
    }
    ...
    function updateSelectedOptionResults(options, fin, isHealthy, commercialImpact, rec) {
        ...
        // Real per-run trade-off analysis / explainability -- computed by
        // the backend (UNAI's real evidence text, or the original
        // Gemini/Groq narrative when that path runs instead) but never
        // previously rendered anywhere in this UI.
        const tradeoffBox = document.getElementById('recTradeoffBox');
        const tradeoffText = document.getElementById('recTradeoffText');
        if (tradeoffBox && tradeoffText) {
            if (rec && rec.tradeOffAnalysis) {
                tradeoffText.textContent = rec.tradeOffAnalysis;
                tradeoffBox.style.display = 'block';
            } else {
                tradeoffBox.style.display = 'none';
            }
        }
        const engineBadge = document.getElementById('recEngineBadge');
        if (engineBadge && rec && rec.engine) engineBadge.textContent = rec.engine;
