...
class SupplyChainManagerAgent:
    ...
    def run_pipeline(
        ...
        return {
            ...
            # Hoisted from recommendations.unaiTokenComparison (see
            # recommendation_agent.py's _consult_unai) -- frontend/app.js's
            # UNAI badge script watches for this field at the TOP level of
            # the /api/pipeline response, same convention as the other
            # Node/TS conversions in this series.
            "unaiTokenComparison": recommendations.get("unaiTokenComparison") if isinstance(recommendations, dict) else None,
            "unaiExplain": recommendations.get("unaiExplain") if isinstance(recommendations, dict) else None,
        }
