...
# the "measured" (vs "estimated") side of unaiIfLlmNarrated in the
# recommendation's token comparison (see backend/agents/unai_bridge.py).
@app.get("/api/unai/key-status")
def unai_key_status():
    key = _os.environ.get("GATEWAY_API_KEY") or _os.environ.get("MISTRAL_API_KEY") or ""
    return {"set": bool(key), "provider": _os.environ.get("GATEWAY_PROVIDER") or ("mistral" if _os.environ.get("MISTRAL_API_KEY") else ""),
            "model": _os.environ.get("GATEWAY_MODEL") or ("mistral-small-latest" if key else ""), "tail": key[-4:] if key else ""}

@app.post("/api/unai/set-key")
async def unai_set_key(body: dict):
    key = str(body.get("key") or "").strip()
    if not key:
        raise HTTPException(status_code=400, detail="provide a key")
    import re
    if re.match(r"^sk-or-", key):
        det = {"base": "https://openrouter.ai/api/v1", "provider": "openrouter", "model": "mistralai/mistral-small"}
    elif re.match(r"^gsk_", key):
        det = {"base": "https://api.groq.com/openai/v1", "provider": "groq", "model": "llama-3.1-8b-instant"}
    elif re.match(r"^AIza", key):
        det = {"base": "https://generativelanguage.googleapis.com/v1beta/openai", "provider": "gemini", "model": "gemini-1.5-flash"}
    elif re.match(r"^sk-", key):
        det = {"base": "https://api.openai.com/v1", "provider": "openai", "model": "gpt-4o-mini"}
    else:
        det = {"base": "https://api.mistral.ai/v1", "provider": "mistral", "model": "mistral-small-latest"}
    _os.environ["GATEWAY_API_KEY"] = key
    _os.environ["GATEWAY_BASE_URL"] = str(body.get("base") or det["base"])
    _os.environ["GATEWAY_MODEL"] = str(body.get("model") or det["model"])
    _os.environ["GATEWAY_PROVIDER"] = det["provider"]
    return {"ok": True, "provider": det["provider"], "model": _os.environ["GATEWAY_MODEL"], "tail": key[-4:]}

@app.post("/api/unai/clear-key")
def unai_clear_key():
    for k in ("GATEWAY_API_KEY", "GATEWAY_BASE_URL", "GATEWAY_MODEL", "GATEWAY_PROVIDER", "MISTRAL_API_KEY"):
