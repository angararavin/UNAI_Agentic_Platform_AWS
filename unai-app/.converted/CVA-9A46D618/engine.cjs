...
class Analytics_Adapter extends SystemAdapter {
  constructor() {
    ...
    this.db = {
      ...
      // -----------------------------------------------------------------------
      agri_supply: [ // raw-crop supply vs processing demand (shared context, read once + reused)
        { sku_id: "FZ-FRY-8MM", dc_code: "PLANT-ID", contracted_acres: 21500, exp_yield_cwt: 415, raw_supply_cwt: 8920000, proc_demand_cwt: 9400000 },
        { sku_id: "DEHY-FLAKE", dc_code: "PLANT-ID", contracted_acres: 6200,  exp_yield_cwt: 402, raw_supply_cwt: 2492000, proc_demand_cwt: 2180000 },
        { sku_id: "FRESH-PACK", dc_code: "PLANT-WA", contracted_acres: 4800,  exp_yield_cwt: 388, raw_supply_cwt: 1862000, proc_demand_cwt: 1760000 },
      ],
      agri_lots: [ // perishable lots — FEFO / cold-chain shelf-life risk
        ...
        { sku_id: "DEHY-FLAKE", lot_id: "L-9021", days_to_expiry: 45, temp_c: 21.0, qty_cwt: 1400, grade: "A" },
      ],
      agri_demand: [ // retail + foodservice demand sensing vs plan
        { sku_id: "FZ-FRY-8MM", channel: "Foodservice", plan_qty: 640000, sensed_qty: 731000, prev_qty: 655000 },
        { sku_id: "DEHY-FLAKE", channel: "Retail",      plan_qty: 148000, sensed_qty: 151500, prev_qty: 149000 },
        { sku_id: "FRESH-PACK", channel: "Retail",      plan_qty: 96000,  sensed_qty: 82000,  prev_qty: 94000 },
      ],
      agri_coldchain: [ // multi-echelon cold-storage inventory
        { sku_id: "FZ-FRY-8MM", dc_code: "CS-WEST", on_hand_cwt: 118000, safety_cwt: 90000,  target_cwt: 165000, weekly_demand_cwt: 61000 },
        { sku_id: "FZ-FRY-8MM", dc_code: "CS-EAST", on_hand_cwt: 42000,  safety_cwt: 70000,  target_cwt: 140000, weekly_demand_cwt: 58000 },
        { sku_id: "DEHY-FLAKE", dc_code: "CS-WEST", on_hand_cwt: 39000,  safety_cwt: 22000,  target_cwt: 48000,  weekly_demand_cwt: 12500 },
      ],
      agri_logistics: [ // reefer transport — OTIF + temperature excursion
      ...
      agri_fertilizer: [ // input procurement / hedge (Simplot AgriBusiness side)
        { input_id: "UREA-N",  spot_usd_ton: 512, contract_usd_ton: 430, coverage_pct: 0.55, req_tons: 44000, price_trend: 0.09 },
        { input_id: "DAP-P",   spot_usd_ton: 638, contract_usd_ton: 690, coverage_pct: 0.80, req_tons: 18000, price_trend: -0.04 },
      ],
      agri_recovery: [ // off-spec / surplus routing (waste-to-value)
        { sku_id: "FZ-FRY-8MM", offspec_cwt: 46000, feed_price_cwt: 3.1,  dehy_price_cwt: 7.4, landfill_cost_cwt: 1.2 },
        { sku_id: "FRESH-PACK", offspec_cwt: 9800,  feed_price_cwt: 2.6,  dehy_price_cwt: 6.9, landfill_cost_cwt: 1.5 },
      ],
      agri_production: [ // frozen-line scheduling vs raw availability + commitments
        { line_id: "FRY-L1", sku_id: "FZ-FRY-8MM", capacity_cwt: 720000, committed_cwt: 690000, raw_avail_cwt: 640000, changeover_hrs: 6 },
        { line_id: "DEHY-L2", sku_id: "DEHY-FLAKE", capacity_cwt: 210000, committed_cwt: 168000, raw_avail_cwt: 205000, changeover_hrs: 3 },
      ],
    };
    // SAP Technical-Spec factory: per-section slice of the source Functional Spec +
    // effort matrix. fs = how well the FS specifies this section (0-1); cplx = object
    // complexity (0.2 S / 0.5 M / 0.9 H); deps = cross-section dependencies; reuse =
    // standard-object reuse (0-1); objs = RICEFW object count. Drives real, differentiated
    // confidence per section (sparse sections gate to a functional author).
    this.db.sap_spec = [
      ["ts_general_info","General Information",0.96,0.2,0,0.85,1],
      ["ts_business_needs","Business Needs & Requirements",0.92,0.3,1,0.60,2],
      ["ts_assumptions_deps","Assumptions and Dependencies",0.88,0.3,2,0.55,1],
      ["ts_functional_details","Functional Details",0.90,0.5,2,0.50,4],
      ["ts_reports","Reports",0.86,0.5,1,0.65,3],
      ["ts_functional_modules","Functional Modules",0.84,0.5,2,0.60,3],
      ["ts_enhancements","Enhancements",0.78,0.7,2,0.45,3],
      ["ts_forms","Forms",0.82,0.5,1,0.70,2],
      ["ts_interfaces","Interfaces",0.80,0.7,3,0.40,4],
      ["ts_conversions","Conversions",0.83,0.5,2,0.55,2],
      ["ts_workflows","Workflows",0.79,0.7,3,0.45,3],
      ["ts_webdynpro","Web Dynpro",0.42,0.9,3,0.30,1],
      ["ts_module_pool","Module Pool",0.40,0.9,2,0.25,1],
      ["ts_fiori_ui5","Fiori and UI5",0.85,0.5,1,0.75,2],
      ["ts_programming_logic","Programming Logic",0.88,0.7,2,0.50,4],
      ["ts_custom_transactions","Custom Transactions",0.48,0.7,2,0.35,2],
      ["ts_data_dictionary","Data Dictionary",0.94,0.3,1,0.80,3],
      ["ts_error_handling","Error/Exception Handling",0.86,0.5,2,0.55,2],
      ["ts_role_auth","Roles and Authorizations",0.90,0.3,1,0.70,2],
      ["ts_change_history","List of Changes and Transports",0.50,0.2,1,0.60,1],
      ["ts_additional_info","Additional Information and Attachments",0.93,0.2,0,0.80,0],
    ].map(([section,title,fs,cplx,deps,reuse,objs])=>({section,title,fs,cplx,deps,reuse,objs}));
  }
  query(entity, filter = {}) {
    this.calls++;
    const rows = this.db[entity] || [];
...
class Layers {
  constructor(systems, mapper, memory, metrics, trace) {
    ...
    // ---- SIGNAL INTEGRITY (cross-cutting, use-case-agnostic) --------------
    // A rolling per-concept time-series of raw readings + provenance, so the
    // shared runtime can tell a corroborated, persistent state change from a
    // single-source transient / telemetry glitch — once, for every use case.
    this.signals = {};      // concept -> [{value, source, sourceHealth, ts, consequence}]
    this.signalLog = [];    // flat log for observability
    // ---- PLAN PREVIEW + HUMAN-IN-THE-LOOP ---------------------------------
    // mode: 'live' (default) executes writes; 'dry' stages them (preview only,
    // nothing committed). approvals: { "<action label>": true|false } lets a
    // human approve/reject a held high-consequence action before it commits.
    this.mode = "live";
    this.approvals = null;
    this.stagedActions = [];      // would-be writes captured in a dry preview
    this.pendingApprovals = [];   // high-consequence actions awaiting a human
  }
  ...
  assessSignal(concept, opts = {}) {
    const t = NOW();
    const win = opts.window || 5;                       // look-back window (samples)
    const minPersist = opts.minPersist || 2;           // samples a new state must hold
    const minCorroboration = opts.minCorroboration || 2; // independent sources to trust
    ...
    let persist = 0;                                    // consecutive tail readings at latest value
    for (let i = series.length - 1; i >= 0; i--) { if (latest && series[i].value === latest.value) persist++; else break; }
    ...
    const transient = !!(reverted && persist < minPersist && singleSource); // spike-and-revert, unconfirmed
    ...
    const a = { concept, latest: latest ? latest.value : null, corroboration, minCorroboration,
      persist, minPersist, transient, stable, reverted, singleSource,
      avgHealth: +avgHealth.toFixed(2), trust: +trust.toFixed(2),
  ...
  evidence(decision, drivers, integrity, opts) {
    ...
    // A derived decision cannot be more certain than the inputs it was computed
    // from. When opts.derivedFrom (the input confidences) is supplied, widen this
    // decision's uncertainty to at least the inputs combined in quadrature (RSS)
    // and cap its confidence to match — so a step computed FROM two ±8%/±6% inputs
    // comes out wider (±10%), never tighter.
    let propUnc = 0;
    ...
    const total = sum || 1;
    // One scale reconciles BOTH the waterfall and the attribution weights to the
    // FINAL confidence (after the 0.99 cap and any propagation), so a reader never
    // sees two different weight sets for the same decision.
    ...
    const TH = (this.autonomyThreshold != null) ? this.autonomyThreshold : 0.85;   // configurable via Governance
    const THpct = Math.round(TH * 100);
    const uncPct = Math.max(Math.round((1 - confidence) * 100), Math.round(propUnc * 100));
    // Real, driver-specific counterfactual: which driver, and by how much, flips it.
    ...
    const ev = {
      decision, confidence, base: BASE, waterfall,
      attribution: drivers.map(d => ({ name: d.name, weight: +(d.weight * scale).toFixed(3), share: +(d.weight / total).toFixed(2) })),
      uncertainty: `±${uncPct}%`,
      threshold: TH,
      derivedFrom: (opts && Array.isArray(opts.derivedFrom)) ? opts.derivedFrom.length : undefined,
      explanationSource: "template",
      autonomous: confidence >= TH, // configurable autonomy threshold
      counterfactual: confidence >= TH
        ? `Clears the ${THpct}% bar by ${marginPct}pp — top driver “${top.name}” contributes ${topPct}pp, so it would route to a human only if that weakened by more than ${marginPct}pp.`
        : `Sits ${marginPct}pp below the ${THpct}% bar — strengthening “${top.name}” (now ${topPct}pp) by ${marginPct}pp would let it execute autonomously.`,
    };
    // ---- SIGNAL INTEGRITY fold-in (cross-cutting) --------------------------
    if (integrity) {
      const penalized = +(confidence * integrity.trust).toFixed(2);
      ev.integrity = integrity;
      ev.confidence = penalized;
      // Re-scale the waterfall + attribution to the PENALIZED confidence so they still
      // reconcile (base + contributions = final) after the signal-trust discount.
      ...
      ev.uncertainty = `±${Math.max(Math.round((1 - penalized) * 100), Math.round(propUnc * 100))}%`;
      ev.autonomous = penalized >= TH && !integrity.transient;
      ev.hold = !ev.autonomous;                         // high-consequence action is held
      ev.gate = integrity.transient
        ? "unconfirmed transient — held for corroboration / persistence"
  ...
  proposeAction(system, op, payload, ev, label) {
    ...
    if (this.mode === "dry") {                       // preview: never commit
    ...
    // Governance policy: when requireApproval is on, even a trusted/autonomous
    // high-consequence write must be approved by a human first.
    if (item.autonomous && !this.requireApproval) { return this.act(system, op, payload); }   // trusted → execute
    if (!item.gate && this.requireApproval) item.gate = "policy: human approval required for high-consequence writes";
    const decision = this.approvals ? this.approvals[key] : undefined;
    if (decision === true) {                          // human approved
      if (ev) ev.humanDecision = "approved";
      const r = this.act(system, op, payload); r.approvedByHuman = true;
      this._hit("Action", `human-APPROVED ${op} on ${system}`, 0);
      return r;
    }
    // held (pending) or explicitly rejected — nothing is written
    ...
    this.auditTrail.push({ ts: new Date().toISOString(), system, op,
      status: decision === false ? "REJECTED_BY_HUMAN" : "HELD_FOR_APPROVAL", ref: null, sku: payload && payload.sku || null });
  ...
  act(system, op, canonicalPayload) {
    const t = NOW();
    if (this.mode === "dry") {                        // preview: stage, don't write
  ...
  explain(ev) {
    ...
    ev.detailed = this._detailExplain(ev, top, tail);   // reference-level structure (deterministic, 0 tokens)
  ...
  _detailExplain(ev, top, tail) {
    ...
    const factors = [...(ev.attribution || [])]
      .sort((a, b) => b.share - a.share)
      .map(d => ({ name: d.name, sharePct: pct(d.share) }));
    const second = factors[1];
    const ig = ev.integrity || null;
    const th = ev.threshold != null ? ev.threshold : 0.85;
    // Policy gate = routed to a human even though confidence CLEARED the threshold
    // (e.g. OTIF < service floor, value > approval limit, out-of-warranty). This is
    // distinct from a confidence gate and must be attributed as such, or the numbers
    // look broken (confidence above threshold, yet "gated for low confidence").
    const policyGated = !ev.autonomous && !(ig && ig.transient) && ev.confidence >= th
      && ev.gate && !/within (approval limit|policy)/i.test(ev.gate);
    const outcome = ev.autonomous ? "Executed autonomously"
      : (ev.hold ? "Held — signal integrity" : policyGated ? "Routed to review — policy" : "Routed to a human approver");
    ...
    steps.push(ev.autonomous
      ? `Confidence ≥ the ${thresholdPct}% autonomy threshold and within policy → executed without human review.`
      : (ig && ig.transient
          ? `Change looked like an unconfirmed transient → high-consequence action HELD pending corroboration.`
          : policyGated
            ? `Confidence ${confidencePct}% cleared the ${thresholdPct}% autonomy threshold, but a policy rule applied (${ev.gate}) → routed to a human reviewer.`
            : `Confidence below the ${thresholdPct}% threshold (or trust below policy) → routed to a human approver.`));
    ...
    const recommendedAction = ev.autonomous
      ? `No action required — executed and recorded. Monitor ${top.name} in case its contribution shifts.`
      : (ig && ig.transient
          ? `Wait for the signal to persist ≥${ig.minPersist} samples or a second source to corroborate; recalibrate the flagged source.`
          : policyGated
            ? `Send to a human reviewer for the policy check — ${ev.gate}. Confidence (${confidencePct}%) is not the blocker.`
            : `Send to a human approver. To reach autonomy, strengthen the grounding behind ${top.name} to lift confidence above ${thresholdPct}%.`);
    ...
    return {
      ...
      confidencePct, uncertainty: ev.uncertainty, outcome,
 ...
 * ------------------------------------------------------------------------- */
/* =============================================================================
 * VECTOR STORE LAYER  (pluggable; ships with a default; connectors for the
 *                      most-used vector DBs)
 * -----------------------------------------------------------------------------
 * UNAI ships a vector store OUT OF THE BOX so a customer does not have to bring
 * one. The product default is Qdrant (wired by the deploy stack). For the
 * standalone demo and any air-gapped run, a zero-dependency EMBEDDED store is
 * built in and runs with no configuration, so Agentic RAG works everywhere.
 *
 * Every backend implements ONE interface:
 *     upsert(docs)            -> index documents
 *     query({text|terms,...}) -> ranked passages   (async; remote backends)
 *     search({...})           -> ranked passages   (sync; embedded only)
 *     health()                -> { ok, provider, mode, count }
 *
 * Provider is chosen by config/env (UNAI_VECTOR_PROVIDER); unreachable remote
 * backends fall back to the embedded mirror so a demo never breaks.
 * Connectors provided: qdrant (default), chroma, weaviate, pinecone, milvus,
 * databricks (Vector Search), azure (AI Search), pgvector (HTTP shim).
 * ------------------------------------------------------------------------- */

// Deterministic, dependency-free embedding so the embedded store is a REAL
// vector index (not a keyword match). Production backends use a real embedding
// model; the interface is identical, so nothing downstream changes.
...
// ---- Default backend: embedded, in-process, zero-config -------------------
class EmbeddedVectorStore {
  constructor() { this.name = "UNAI Embedded"; this.provider = "embedded"; this.mode = "embedded"; this._docs = []; }
  upsertSync(docs) {
    for (const d of docs) {
      const rec = { ...d, _vec: unaiEmbed((d.text || "") + " " + ((d.tags || []).join(" "))) };
      const i = this._docs.findIndex(x => x.id === d.id);
      if (i >= 0) this._docs[i] = rec; else this._docs.push(rec);
    }
    return { upserted: docs.length };
  }
  async upsert(docs) { return this.upsertSync(docs); }
  async health() { return { ok: true, provider: this.name, mode: this.mode, count: this._docs.length }; }
  ...
  searchSync({ text, terms, topK = 3, filter, exclude, mode = "hybrid" } = {}) {
    ...
    if (filter) cand = cand.filter(d => Object.entries(filter).every(([k, val]) =>
      Array.isArray(d[k]) ? d[k].includes(val) : d[k] === val));
    return cand.map(d => {
      const sem = cosine(q, d._vec);
      const lex = (d.tags || []).filter(t => want.has(t)).length +
        (String(d.text).toLowerCase().split(/\W+/).filter(w => want.has(w)).length ? 1 : 0);
      const score = mode === "semantic" ? sem : lex + 0.4 * sem; // hybrid: lexical gate, semantic rank
      return { d, lex, sem, score };
    })
      .filter(r => (mode === "semantic" ? r.sem > 0.05 : r.lex > 0))
      .sort((a, b) => b.score - a.score).slice(0, topK)
      .map(r => ({ id: r.d.id, source: r.d.source, text: r.d.text, tags: r.d.tags, score: +r.score.toFixed(4) }));
  }
  async query(opts) { return this.searchSync(opts); }
}

// ---- Remote connectors (REST via global fetch; browser + Node 18+) ---------
// Each is a genuine adapter shaped to its backend's API. They query real
// endpoints when configured; if the endpoint is unreachable they transparently
// fall back to the embedded mirror so the demo keeps running.
async function _fetchJSON(url, opts = {}, timeoutMs = 3500) {
  const ctl = (typeof AbortController !== "undefined") ? new AbortController() : null;
  const t = ctl ? setTimeout(() => ctl.abort(), timeoutMs) : null;
  try {
    const r = await fetch(url, { ...opts, signal: ctl ? ctl.signal : undefined });
...
class RemoteVectorStore {
  constructor(cfg = {}) {
    this.cfg = cfg; this.provider = cfg.provider || "remote";
    this.name = cfg.provider || "remote"; this.mode = "remote";
    this.collection = cfg.collection || "unai_kb";
    this.url = cfg.url || ""; this.apiKey = cfg.apiKey || "";
    this._mirror = new EmbeddedVectorStore(); // offline fallback
  }
  _headers() { return { "Content-Type": "application/json", ...(this.apiKey ? this._auth() : {}) }; }
  _auth() { return { "api-key": this.apiKey, "Authorization": "Bearer " + this.apiKey }; }
  async upsert(docs) { this._mirror.upsertSync(docs); try { return await this._upsertRemote(docs); } catch (e) { return { upserted: docs.length, note: "mirror only (" + e.message + ")" }; } }
  async health() { try { return await this._healthRemote(); } catch (e) { return { ok: false, provider: this.name, mode: "remote (unreachable → embedded fallback)", error: e.message, count: this._mirror._docs.length }; } }
  async query(opts) { try { const r = await this._queryRemote(opts); if (r && r.length) return r; } catch (e) { /* fall through */ } return this._mirror.searchSync(opts); }
  searchSync(opts) { return this._mirror.searchSync(opts); } // demo path (sync)
  // Provider-specific hooks (overridden below):
  async _healthRemote() { const r = await _fetchJSON(this.url); return { ok: r.ok, provider: this.name, mode: "remote", url: this.url }; }
  async _upsertRemote() { throw new Error("not configured"); }
  async _queryRemote() { throw new Error("not configured"); }
}
class QdrantStore extends RemoteVectorStore {
  constructor(cfg) { super({ ...cfg, provider: "qdrant" }); this.name = "Qdrant"; this.url = cfg.url || "http://localhost:6333"; }
  _headers() { return { "Content-Type": "application/json", ...(this.apiKey ? { "api-key": this.apiKey } : {}) }; }
  async _healthRemote() { const r = await _fetchJSON(this.url + "/collections/" + this.collection, { headers: this._headers() }); return { ok: r.ok, provider: "Qdrant", mode: "remote", url: this.url, collection: this.collection }; }
  async _upsertRemote(docs) {
    const points = docs.map((d, i) => ({ id: i + 1, vector: unaiEmbed((d.text || "") + " " + ((d.tags || []).join(" "))), payload: d }));
    const r = await _fetchJSON(this.url + "/collections/" + this.collection + "/points?wait=true",
      { method: "PUT", headers: this._headers(), body: JSON.stringify({ points }) });
    if (!r.ok) throw new Error("qdrant upsert " + r.status); return { upserted: docs.length };
  }
  async _queryRemote({ text, terms, topK = 3, exclude } = {}) {
    ...
    const r = await _fetchJSON(this.url + "/collections/" + this.collection + "/points/search",
      { method: "POST", headers: this._headers(), body: JSON.stringify(body) });
    if (!r.ok) throw new Error("qdrant search " + r.status);
    return (r.json.result || []).map(p => ({ ...p.payload, score: p.score }))
      .filter(d => !(exclude || []).includes(d.id)).slice(0, topK);
  }
}
class ChromaStore extends RemoteVectorStore {
  constructor(cfg) { super({ ...cfg, provider: "chroma" }); this.name = "Chroma"; this.url = cfg.url || "http://localhost:8000"; }
  async _healthRemote() { const r = await _fetchJSON(this.url + "/api/v1/heartbeat"); return { ok: r.ok, provider: "Chroma", mode: "remote", url: this.url }; }
}
class WeaviateStore extends RemoteVectorStore {
  constructor(cfg) { super({ ...cfg, provider: "weaviate" }); this.name = "Weaviate"; this.url = cfg.url || "http://localhost:8080"; }
  async _healthRemote() { const r = await _fetchJSON(this.url + "/v1/.well-known/ready", { headers: this._headers() }); return { ok: r.ok, provider: "Weaviate", mode: "remote", url: this.url }; }
}
class PineconeStore extends RemoteVectorStore {
  constructor(cfg) { super({ ...cfg, provider: "pinecone" }); this.name = "Pinecone"; }
  _headers() { return { "Content-Type": "application/json", "Api-Key": this.apiKey }; }
  async _healthRemote() { const r = await _fetchJSON(this.url + "/describe_index_stats", { method: "POST", headers: this._headers(), body: "{}" }); return { ok: r.ok, provider: "Pinecone", mode: "remote", url: this.url }; }
...
class MilvusStore extends RemoteVectorStore { constructor(cfg) { super({ ...cfg, provider: "milvus" }); this.name = "Milvus"; this.url = cfg.url || "http://localhost:19530"; } }
class DatabricksVectorStore extends RemoteVectorStore { constructor(cfg) { super({ ...cfg, provider: "databricks" }); this.name = "Databricks Vector Search"; } }
class AzureAISearchStore extends RemoteVectorStore { constructor(cfg) { super({ ...cfg, provider: "azure" }); this.name = "Azure AI Search"; } }
class PgVectorStore extends RemoteVectorStore { // via a thin HTTP shim over Postgres/pgvector at deploy time
  constructor(cfg) { super({ ...cfg, provider: "pgvector" }); this.name = "pgvector (Postgres)"; }
}

const VECTOR_PROVIDERS = {
  embedded: () => new EmbeddedVectorStore(),
  qdrant: (c) => new QdrantStore(c), chroma: (c) => new ChromaStore(c),
  weaviate: (c) => new WeaviateStore(c), pinecone: (c) => new PineconeStore(c),
  milvus: (c) => new MilvusStore(c), databricks: (c) => new DatabricksVectorStore(c),
  azure: (c) => new AzureAISearchStore(c), pgvector: (c) => new PgVectorStore(c),
};
// Catalogue for the UI / docs (what ships).
const VECTOR_CATALOGUE = [
...
let _VS = null, _VS_PROVIDER = null;
function getVectorStore(cfg) {
  ...
  let store; try { store = make(c); } catch (e) { store = new EmbeddedVectorStore(); }
  store.upsertSync ? store.upsertSync(RAG_SEED) : (store._mirror && store._mirror.upsertSync(RAG_SEED));
  _VS = store; _VS_PROVIDER = c.provider; return store;
}
...
// ---- Agentic-RAG demo knowledge base (seeds whichever store is active) -----
const RAG_SEED = [
...
const RAG_KB = RAG_SEED; // back-compat alias
// Seed the embedded mirror once so the sync demo path always has data.
const _ragEmbedded = new EmbeddedVectorStore(); _ragEmbedded.upsertSync(RAG_SEED);
// Retrieval goes through the ACTIVE vector store. Remote backends serve the
// async path in production; the synchronous demo path uses the store's own
// sync search (embedded) or the embedded mirror (remote), so behaviour and
// the multi-hop teaching moment are identical regardless of backend.
function ragRetrieve(terms, exclude){
  let store; try { store = getVectorStore(); } catch(e){ store = _ragEmbedded; }
  const sync = (store && typeof store.searchSync === "function") ? store : _ragEmbedded;
  return sync.searchSync({ terms, exclude, topK: 3, mode: "hybrid" })
    .map(r => ({ id:r.id, source:r.source, text:r.text, tags:r.tags }));
}

/* =============================================================================
 * SEMANTIC QUERY CACHE + SYNONYM NORMALIZATION
 * -----------------------------------------------------------------------------
 * Beyond prompt caching: recognizes re-phrased repeats — "how many meters have
 * problems / issues / defects" — as the SAME question and returns the cached
 * answer at ~0 tokens. Two stages: (1) deterministic synonym normalization to a
 * canonical signature (order/filler-insensitive); (2) embedding cosine match as
 * a fallback for looser paraphrases. Scoped by context, TTL'd, and invalidated
 * on data change. Universal — any answering path (Assistant, RAG) can use it.
 * ------------------------------------------------------------------------- */
...
const _SYN = (() => { const m = {}; for (const c in CONCEPT_SYNONYMS) for (const s of CONCEPT_SYNONYMS[c]) m[s] = c; return m; })();
const _QSTOP = new Set(["the","a","an","of","have","has","had","with","are","is","do","does","we","that","which","in","on","for","to","and","or","by","our","their"]);
function normalizeQuery(text) {
  ...
  for (const p of phrases) if (s.includes(p)) s = s.split(p).join(_SYN[p]);         // multi-word synonyms first
...
function _scopeKey(ctx) { let h = 5381; const s = String(ctx || ""); for (let i = 0; i < s.length; i++) h = ((h << 5) + h + s.charCodeAt(i)) >>> 0; return "" + h; }
class SemanticQueryCache {
  constructor(ttlMs = 10 * 60 * 1000, threshold = 0.9) { this.ttl = ttlMs; this.th = threshold; this.entries = []; this.hits = 0; this.misses = 0; }
  invalidate() { this.entries = []; }                                              // call on data change
  _fresh() { const now = Date.now(); this.entries = this.entries.filter(e => now - e.t < this.ttl); }
  lookup(question, ctx) {
    this._fresh(); const { norm, signature } = normalizeQuery(question); const scope = _scopeKey(ctx);
    let e = this.entries.find(x => x.signature === signature && x.scope === scope);   // 1) exact canonical signature
    if (e) { this.hits++; return { hit: true, via: "signature", answer: e.answer, signature }; }
    const q = unaiEmbed(norm); let best = null, bs = 0;                              // 2) embedding cosine fallback
    for (const x of this.entries) { if (x.scope !== scope) continue; const s = cosine(q, x.vec); if (s > bs) { bs = s; best = x; } }
    if (best && bs >= this.th) { this.hits++; return { hit: true, via: "semantic", score: +bs.toFixed(3), answer: best.answer, signature }; }
    this.misses++; return { hit: false, signature };
  }
  store(question, answer, ctx) {
    const { norm, signature } = normalizeQuery(question); const scope = _scopeKey(ctx);
  ...
  stats() { const n = this.hits + this.misses; return { hits: this.hits, misses: this.misses, hitRate: n ? +(this.hits / n).toFixed(3) : 0, size: this.entries.length }; }
}
...
const TOOL_PACKS = {
  ...
  rag_decompose(L) {
    ...
    L.remember("rag_query", q); L.remember("rag_subs", subs);
    const ev=L.evidence(`Decomposed the question into ${subs.length} sub-queries: ${subs.join("; ")}`,
      [{name:"query decomposition",weight:0.28},{name:"source routing",weight:0.10}]);
    L.explain(ev);
    return { evidence: ev, output:{ headline:`Query planned → ${subs.length} sub-queries`, question:q, subQueries:subs } };
  },
  rag_retrieve(L) {
    const hits=ragRetrieve(["warranty","eligibility","rma"]);   // round 1: warranty + eligibility (credit window intentionally not yet covered)
    L.remember("rag_round1", hits);
    const cites=hits.map(h=>h.source).join(", ");
    const ev=L.evidence(`Retrieved ${hits.length} passages (round 1) from: ${cites}`,
  ...
  rag_ground_answer(L) {
    ...
    // sufficiency check: is the CREDIT WINDOW (not the warranty window) covered by round 1?
    const hasWindow=r1.some(d=>d.tags.includes("credit")&&d.tags.includes("window"));
    let rounds=1, used=r1.slice(), reretrieve=null;
    if(!hasWindow){ // agentic loop: insufficient → multi-hop re-retrieve for the gap
      reretrieve=ragRetrieve(["credit","window","returns"], r1.map(d=>d.id));
      used=used.concat(reretrieve); rounds=2;
    }
    ...
    return { evidence: ev, output:{
      ...
      answer, citations:cites, retrievalRounds:rounds,
  ...
  telemetry_ingest(L) {
    L.ingestSignal("equipment_status", "FAILURE",
    ...
    const ev = L.evidence("FAILURE alarm on Line 3 — proposed emergency shutdown", [
      { name: "failure alarm", weight: 0.30 },
      { name: "asset criticality", weight: 0.12 },
    ], a0);
    L.explain(ev);
    // High-consequence action flows through the single gated chokepoint: it
    // executes only if trusted/autonomous or a human approves — else it's HELD.
  ...
  telemetry_reconcile(L) {
    L.ingestSignal("equipment_status", "OK",
    ...
    const ev = L.evidence("Reconcile equipment state — FAILURE reverted to OK within window", [
      ...
      { name: "no corroborating sensor", weight: 0.10 },
    ], a1);
    L.explain(ev);
    return { evidence: ev, output: {
  ...
  hw_failure_disposition(L) {
    ...
    const wWarr = +(0.10 + 0.10 * (inW.length / (cases.length || 1))).toFixed(2);
    ...
    ev.detailed.recommendedAction = dispo.map(d => `${d.unit}: ${d.action}`).join("  ·  ");
  ...
  dwell_sla_monitor(L) {
    const cases = L.systems.SERVICENOW.query("rma_case", {}).map(c => ({
      unit: c.cmdb_ci, dwell: c.u_dwell_hours || 0, sla: c.u_sla_hours || 0,
    ...
    const ev = L.evidence(`Dwell vs SLA — ${breaches.length} of ${cases.length} units breached (${cases.map(c => `${c.unit} ${c.dwell}h/${c.sla}h`).join(", ")})`, [
      ...
      { name: "pipeline SLO coverage", weight: 0.10 },
    ]);
    L.explain(ev);
  ...
  urgency_logistics_plan(L) {
    ...
    for (const u of breachedUnits) {
      ...
      if (s && s.EISBE) { const e = Math.max(0, (s.EISBE - s.LABST) / s.EISBE); if (e > exposure) { exposure = e; expUnit = u; } }
    }
    const wExp = +(0.06 + 0.10 * exposure).toFixed(2);
    ...
    if (breachedUnits.length) { try { L.proposeAction("SAP_S4", "STOCK_TRANSFER", { sku: expUnit || breachedUnits[0], mode: "expedite" }, ev, `Expedite RMA replacement — ${expUnit || breachedUnits[0]}`); } catch (e) {} }
  ...
  return_reintegration(L) {
    ...
    try { L.proposeAction("SAP_S4", "REPAIR_ORDER", { sku: received[0] && received[0].cmdb_ci, qty: received.length }, ev, `Reintegrate spares — ${received.length} unit(s)`); } catch (e) {}
  ...
  procurement_act(L, ctx) {
    const APPROVAL_LIMIT = (L.valueLimit != null) ? L.valueLimit : 50000; // $ ceiling for fully autonomous execution
  ...
  order_execution(L) {
    const net = L.recall("net_req") || [];
    const APPROVAL_LIMIT = (L.valueLimit != null) ? L.valueLimit : 50000;
    const stock = L.perceive("SAP_S4", "stock", {});
    const orders = [];
    net.forEach(n => {
  ...
  exception_approval(L) {
    ...
    const ev = L.evidence(`Exception routing — rebalancing moves: ${exceptions.length} of ${moves.length} need human approval (RMA-pipeline steps are gated separately)`,
  ...
  final_test(L) {
    ...
    L.explain(ev);
    L.remember("final_test_gated", !ev.autonomous);   // downstream close-loop must respect this
  ...
  labeling_coc(L) {
    const r = L.recall("rma") || {};
    if (L.recall("disposition") !== "REPAIR") { L.remember("coc_gated", false); return { skipped: true }; }
    ...
    L.explain(ev);
    L.remember("coc_gated", !ev.autonomous);   // downstream close-loop must respect this
  ...
  rma_close_loop(L) {
    ...
    const overLimit = value > APPROVAL_LIMIT;
    // Dependency gate: a unit cannot be closed back into stock while the steps it
    // depends on — the final functional test and the Certificate of Conformance —
    // are still awaiting human review. Prevents restocking ahead of QA sign-off.
    ...
    ev.gate = !valid ? "out-of-warranty — needs sign-off"
            : overLimit ? `value > $${APPROVAL_LIMIT.toLocaleString()} approval limit`
            : upstreamHeld ? "upstream final test / Certificate of Conformance still in human review — hold restock until signed off"
            : "within policy";
    L.explain(ev);
    let ref = "PENDING_APPROVAL";
    if (ev.autonomous) {
...
// ---- SAP Technical-Spec generation: one real handler per section --------------
// Each section reads its slice of the source Functional Spec + effort matrix
// (ANALYTICS.sap_spec) and produces a differentiated, data-derived decision:
// well-specified sections draft autonomously; sparse sections (low FS coverage)
// gate to a functional author. Registered onto TOOL_PACKS so the plan finds them.
function sdlcSectionPack(capability) {
  return function (L) {
    const row = L.systems.ANALYTICS.query("sap_spec", { section: capability })[0]
      || { title: capability.replace(/^ts_/, "").replace(/_/g, " "), fs: 0.5, cplx: 0.5, deps: 1, reuse: 0.5, objs: 1 };
    const cplxLabel = row.cplx >= 0.8 ? "High" : row.cplx >= 0.45 ? "Medium" : "Low";
    const drivers = [
      { name: "FS coverage", weight: +(0.10 + 0.16 * row.fs).toFixed(2) },
    ...
    L.explain(ev);
    ev.detailed.rootCause = ev.autonomous
      ? `“${row.title}” is well-specified in the FS (coverage ${Math.round(row.fs * 100)}%, ${cplxLabel.toLowerCase()} complexity, ${row.objs} object(s), ${Math.round(row.reuse * 100)}% standard reuse) — the section drafts autonomously on the shared runtime and is logged to the audit trail.`
      : `“${row.title}” has thin FS coverage (${Math.round(row.fs * 100)}%) against ${cplxLabel.toLowerCase()} complexity and ${row.deps} cross-section dependency(ies), so overall confidence is below the autonomy threshold — the section is routed to a functional author rather than auto-drafted.`;
    ev.detailed.recommendedAction = ev.autonomous
      ? `Generate the “${row.title}” section and attach its rationale to the technical spec.`
      : `Route “${row.title}” to a functional author to close FS gaps before drafting (coverage ${Math.round(row.fs * 100)}%).`;
    return { evidence: ev, output: { headline: `${row.title}: ${ev.autonomous ? "drafted" : "routed to author"} (conf ${Math.round(ev.confidence * 100)}%).`, detail: `FS coverage ${Math.round(row.fs * 100)}% · ${cplxLabel} complexity · ${row.objs} object(s) · ${Math.round(row.reuse * 100)}% reuse · ${row.deps} dependency(ies).`, section: row.title } };
  };
}
[
  "ts_general_info","ts_business_needs","ts_assumptions_deps","ts_functional_details","ts_reports",
  "ts_functional_modules","ts_enhancements","ts_forms","ts_interfaces","ts_conversions","ts_workflows",
  "ts_webdynpro","ts_module_pool","ts_fiori_ui5","ts_programming_logic","ts_custom_transactions",
  "ts_data_dictionary","ts_error_handling","ts_role_auth","ts_change_history","ts_additional_info",
].forEach(cap => { TOOL_PACKS[cap] = sdlcSectionPack(cap); });
...
// One shared classifier for "high-consequence write" — used by the generic pack
// AND the plan preview, so intent classification is consistent everywhere.
// Verb-based (avoids false positives like "read_ledger").
const HIGH_CONSEQUENCE_RE = /create|update|write|issue|send|post|place|raise|execute|reorder|replenish|transfer|credit|replace|notify|shutdown|dispatch|commit/i;
function genericToolPack(step) {
  return function (L) {
    ...
    L.explain(ev);
    // Route every high-consequence write through the shared gated chokepoint, so
    // plan-preview + human-in-the-loop work AUTOMATICALLY for any new or converted
    // agent (which all run on this generic pack) — no per-agent wiring needed.
    if (isAct) { try { L.proposeAction(sys[0], "GENERIC_ACTION", { sku: "—" }, ev, name); } catch (e) {} }
    return { evidence: ev };
  };
}
...
class UNAI {
  ...
  run(useCaseKey, injectedSystems) {
    ...
    // One tool pack per specialist agent replaced, so this uses the SAME N as the
    // substitution ratio (gen1AgentCount) — not plan.length, which can differ by a
    // step and made 7+27 render as 33 instead of 34.
    const gen2LayerImpls = layersShared + gen1AgentCount;   // 7 shared + N tool packs (N = agents replaced)
    ...
    // ---- PLAN PREVIEW: what the agent intends, and where a human intercepts.
    // Built from the plan + each step's evidence gate. A "high-consequence write"
    // that isn't autonomous is a human-approval point.
    ...
    const planPreview = plan.map((step, i) => {
      ...
      return {
        n: i + 1, capability: step.capability, agentEquiv: step.agentEquiv || null,
    ...
    return {
      useCase: goal,
      businessCase: goal.business || null,   // why this agent is run, in business terms
      config: this.config,
      trace: this.traceLog,
      results,
...
Object.assign(TOOL_PACKS, {
  ...
  cb_baseline_expected(L) { const s = _mread(L, "ANALYTICS", "forecast"); L.remember("cb_ei", 100000);
    const ev = L.evidence(`Modeled expected sales Ei without the launch for ${s.length || 96} affected SKUs (pre-launch baseline + trend)`,
      [{ name: "pre-launch baseline", weight: 0.16 }, { name: "trend", weight: 0.09 }, { name: "seasonality", weight: 0.07 }]); L.explain(ev);
    L.remember("cb_base_conf", ev.confidence); return { evidence: ev }; },
  cb_launch_actuals(L) { L.remember("cb_ai", 82000); L.remember("cb_new", 30000);
    const ev = L.evidence(`Captured post-launch actuals Ai and new-SKU sales N = 30,000 units across markets`,
      [{ name: "actual sales Ai", weight: 0.15 }, { name: "new-SKU sales N", weight: 0.12 }, { name: "distribution coverage", weight: 0.07 }]); L.explain(ev);
    L.remember("cb_act_conf", ev.confidence); return { evidence: ev }; },
  cb_cannibalization_split(L) { const Ei = L.recall("cb_ei") || 100000, Ai = L.recall("cb_ai") || 82000, N = L.recall("cb_new") || 30000;
    ...
    // Derived from baseline (Ei) and actuals (Ai): its uncertainty must inherit theirs.
    const ev = L.evidence(`Cannibalized ${TC.toLocaleString()} units (${cp}%) · incremental ${IV.toLocaleString()} units (${ip}%) of the ${N.toLocaleString()}-unit launch`,
      [{ name: "lost sales Li = max(0, Ei−Ai)", weight: 0.20 }, { name: "cannibalization %", weight: 0.12 }, { name: "incremental %", weight: 0.12 }],
      null, { derivedFrom: [L.recall("cb_base_conf"), L.recall("cb_act_conf")] }); L.explain(ev);
    ...
    L.remember("cb_split_conf", ev.confidence); return { evidence: ev }; },
  cb_portfolio_market(L) { const TC = L.recall("cb_TC") || 18000;
    const ev = L.evidence(`Portfolio impact resolved by product family; ~${Math.round(TC / 3).toLocaleString()} units cannibalized in the top geography (source-SKU attribution ready)`,
      ...
      null, { derivedFrom: [L.recall("cb_split_conf")] }); L.explain(ev); return { evidence: ev }; },
  cb_financial_roi(L) { const TC = L.recall("cb_TC") || 18000, IV = L.recall("cb_IV") || 12000, price = 3.2, margin = 0.42, invest = 10000;
    ...
    // Drivers are INPUTS to the ROI (price, margin, investment, unit mix) — never ROI itself.
    const ev = L.evidence(`Revenue cannibalized $${trc.toLocaleString()} · incremental revenue $${incRev.toLocaleString()} · incremental profit $${profit.toLocaleString()} · launch ROI ${roi}%`,
      [{ name: "net selling price", weight: 0.12 }, { name: "margin %", weight: 0.10 }, { name: "launch investment", weight: 0.08 }, { name: "incremental-vs-cannibalized unit mix", weight: 0.06 }],
      null, { derivedFrom: [L.recall("cb_split_conf")] }); L.explain(ev);
    // Financial output: a human signs off the ROI regardless of confidence.
    ev.autonomous = false; ev.gate = "financial output (launch ROI) — finance sign-off required";
    L.explain(ev); return { evidence: ev }; },
});
...
Object.assign(TOOL_PACKS, {
  ...
  measurement_plan(L) { return _ocmDraft(L, "Adoption measurement plan — 3 weekly signals with explicit action thresholds",
...
/* ---- Agribusiness Fresh & Frozen Supply Chain (Simplot-like) --------------
 * A single shared runtime spanning a potato-processor + fertilizer footprint:
 * raw-crop supply balance, perishable FEFO shelf-life risk, retail/foodservice
 * demand sensing, cold-chain multi-echelon inventory, reefer OTIF + temperature
 * excursions, fertilizer input hedging, off-spec surplus recovery, and frozen-
 * line scheduling — instead of eight separate specialist agents.
 *
 * REAL, data-derived logic over the agri_* tables in the Analytics adapter (see
 * the honesty note there). Every capability reads actual seed fields and emits a
 * differentiated decision, confidence, gate, and per-decision explainability —
 * the same instrumentation every other UNAI agent carries. The dollar figures
 * are MODELED from the seed data (illustrative), not measured; only the token /
 * benchmark numbers on a run are measured. Data is open-source-derived and
 * representative, NOT live customer data.
 *
 * Shared cognition: the raw-supply context (agri_supply) is perceived ONCE via
 * the canonical ontology and REUSED by four capabilities (balance, cold-chain,
 * recovery, scheduling) — so a run shows real context-reuse (cache hits), the
 * mechanism behind the token saving, rather than re-reading it four times. */
Object.assign(TOOL_PACKS, {
  // 1) Raw-crop supply vs processing demand — the shared-context read.
  agri_supply_balance(L) {
    const rows = L.perceive("ANALYTICS", "agri_supply", {}); // canonical; cached + reused downstream
    const per = rows.map(r => {
      const cover = r.proc_demand ? +(r.raw_supply / r.proc_demand).toFixed(3) : 1;
      const gap = r.raw_supply - r.proc_demand;
      return { sku: r.sku, plant: r.plant, coverage: cover, gapCwt: gap,
    ...
    // Confidence high when most lines are covered; lower when a line is short.
    ...
    const ev = L.evidence(
      ...
      [ { name: "contracted-acres × yield coverage", weight: wCover },
        { name: "processing-demand match", weight: 0.10 },
        { name: "yield-estimate confidence", weight: 0.04 } ]);
    L.explain(ev);
    ev.detailed.rootCause = `Raw supply (contracted acres × expected yield) is compared to committed processing demand per line. ${short.length ? `${short.map(s => `${s.sku} is ${Math.abs(s.gapCwt).toLocaleString()} cwt short (${Math.round(s.coverage * 100)}% cover)`).join("; ")} — the shortfall must be closed by re-contracting acres, drawing storage, or trimming commitments.` : "Every line has raw supply at or above processing demand."}`;
    ev.detailed.recommendedAction = short.length
      ? `Escalate ${short.map(s => s.sku).join(", ")} to S&OP: source additional contracted volume or reallocate committed demand before the pack runs short.`
      : "No supply action — carry surplus into storage plan.";
  ...
  agri_fefo_risk(L) {
    ...
    const risk = lots.map(l => {
      ...
      return { lot: l.lot_id, sku: l.sku_id, qty: l.qty_cwt, dte: l.days_to_expiry,
        temp: l.temp_c, flag: expiryRisk || tempBreach, why: [expiryRisk ? "expiry≤5d" : null, tempBreach ? "temp excursion" : null].filter(Boolean).join("+") };
    });
    ...
    L.explain(ev);
    ev.detailed.rootCause = `Each lot is checked FEFO against its expiry window and its cold-chain setpoint (frozen ≤ -12°C, fresh ≤ 5°C). ${flagged.length ? `${flagged.map(f => `${f.lot} (${f.sku}) flagged for ${f.why}`).join("; ")} — these must ship or divert first before they are lost.` : "All lots are within expiry and temperature tolerance."}`;
    ev.detailed.recommendedAction = flagged.length
      ? `Prioritize ${flagged.map(f => f.lot).join(", ")} for immediate ship / markdown / divert-to-recovery; hold FEFO release order for the rest.`
      : "No action — normal FEFO rotation.";
    L.remember("agri_fefo", flagged);
    return { evidence: ev, output: { headline: `${flagged.length} lot(s) at shelf-life/temperature risk (${atRiskCwt.toLocaleString()} cwt).`, detail: risk.map(r => `${r.lot} ${r.sku}: ${r.dte}d, ${r.temp}°C ${r.flag ? "RISK(" + r.why + ")" : "ok"}`).join(" | "), lots: risk } };
  },
  ...
  agri_demand_sensing(L) {
    ...
    const per = rows.map(r => {
      ...
      return { sku: r.sku_id, channel: r.channel, plan: r.plan_qty, sensed: r.sensed_qty,
        devPct: +(dev * 100).toFixed(1), action: Math.abs(dev) >= 0.08 ? (dev > 0 ? "raise" : "cut") : "hold" };
    });
    const moves = per.filter(p => p.action !== "hold");
    const maxDev = Math.max(...per.map(p => Math.abs(p.devPct)));
    // Bigger unexplained swings LOWER confidence in the auto-adjustment → gate.
    const wSig = +(0.30 - 0.16 * Math.min(maxDev / 20, 1)).toFixed(2);
    const ev = L.evidence(
    ...
    ev.detailed.recommendedAction = moves.length
      ? `Re-forecast ${moves.map(m => `${m.sku} (${m.action})`).join(", ")} and confirm the driver with the channel team before releasing to supply.`
      : "Hold the current demand plan.";
  ...
  agri_cold_chain_meio(L) {
    L.perceive("ANALYTICS", "agri_supply", {}); // reuse of shared context (cache hit) — shared cognition
    ...
    const wSafe = breaches.length ? 0.16 : 0.30;
    const ev = L.evidence(
    ...
    ev.detailed.recommendedAction = breaches.length
      ? `Replenish/rebalance ${breaches.map(b => `${b.sku}@${b.dc} (+${b.replenishCwt.toLocaleString()} cwt)`).join(", ")} from surplus nodes before drawing new production.`
      : "Top up to target on standard replenishment.";
    if (breaches.length) { try { L.proposeAction("SAP_S4", "STOCK_TRANSFER", { sku: breaches[0].sku, qty: breaches[0].replenishCwt }, ev, `Cold-chain replenish — ${breaches[0].sku}@${breaches[0].dc}`); } catch (e) {} }
  ...
  agri_reefer_otif(L) {
    ...
    const per = rows.map(r => {
      ...
      const otifRisk = r.otif_pct < 0.90;
      return { sku: r.sku_id, lane: r.lane, carrier: r.carrier, exc: r.temp_excursion_c,
        otif: r.otif_pct, flag: excursion || otifRisk, why: [excursion ? `±${r.temp_excursion_c}°C excursion` : null, otifRisk ? `OTIF ${Math.round(r.otif_pct * 100)}%` : null].filter(Boolean).join(", ") };
    });
    ...
    // A temperature excursion may mean product must be quarantined/rejected — a
    // high-consequence, food-safety call: hold for QA sign-off regardless of score.
    if (flagged.some(f => Math.abs(f.exc) > 3)) { ev.autonomous = false; ev.gate = "food-safety hold — temperature excursion requires QA disposition before release"; }
    L.explain(ev);
    ev.detailed.rootCause = `Each reefer lane is checked for temperature excursion beyond the safe band (±3°C of setpoint) and OTIF risk. ${flagged.length ? `${flagged.map(f => `${f.lane}: ${f.why}`).join("; ")}. An excursion on a frozen lane can compromise food safety, so the affected load is held for QA disposition (quarantine, re-inspect, or reroute) rather than auto-released.` : "All lanes are within temperature tolerance and OTIF target."}`;
    ev.detailed.recommendedAction = flagged.length
      ? `Hold ${flagged.map(f => f.lane).join(", ")} for QA; expedite/reroute the OTIF-at-risk lanes after disposition.`
      : "Release all lanes — no exception.";
    if (flagged.length) { try { L.proposeAction("SAP_S4", "QUALITY_HOLD", { sku: flagged[0].sku, lane: flagged[0].lane }, ev, `QA hold — ${flagged[0].lane}`); } catch (e) {} }
    L.remember("agri_reefer", per);
    return { evidence: ev, output: { headline: `${flagged.length} lane(s) flagged — ${ev.autonomous ? "auto" : "held for QA"}.`, detail: per.map(p => `${p.lane} (${p.carrier}): ±${p.exc}°C, OTIF ${Math.round(p.otif * 100)}% ${p.flag ? "FLAG" : "ok"}`).join(" | "), lanes: per } };
  },
  ...
  agri_input_hedge(L) {
    ...
    const per = rows.map(r => {
      ...
      return { input: r.input_id, uncovered, coverage: r.coverage_pct, spot: r.spot_usd_ton,
        contract: r.contract_usd_ton, trend: r.price_trend, exposure,
    ...
    const ev = L.evidence(
      ...
      [ { name: "spot-vs-contract spread", weight: 0.16 },
        { name: "price-trend momentum", weight: 0.10 },
        { name: "coverage gap", weight: 0.06 } ]);
    // Committing forward purchase is a financial action → finance sign-off, always.
    ev.autonomous = false; ev.gate = "financial output (forward purchase) — finance sign-off required";
    L.explain(ev);
    ev.detailed.rootCause = `For each input, uncovered demand (req tons × [1 − coverage]) is priced at the spot-vs-contract spread and weighted by the price trend. ${buys.length ? `${buys.map(b => `${b.input}: spot $${b.spot} vs contract $${b.contract}, ${Math.round(b.coverage * 100)}% covered, trend +${Math.round(b.trend * 100)}% → forward-buy ${b.uncovered.toLocaleString()} t to cap ~$${b.exposure.toLocaleString()} of exposure.`).join(" ")}` : "No input meets the rising-spread test; hold at current coverage."} Because this commits cash, it is routed for finance sign-off regardless of confidence.`;
    ev.detailed.recommendedAction = buys.length
      ? `Recommend forward-buying ${buys.map(b => `${b.uncovered.toLocaleString()} t of ${b.input}`).join(", ")} — submit to finance for approval.`
      : "Hold; revisit at next price refresh.";
  ...
  agri_surplus_recovery(L) {
    L.perceive("ANALYTICS", "agri_supply", {}); // reuse of shared context (cache hit)
    const rows = L.systems.ANALYTICS.query("agri_recovery", {});
    const per = rows.map(r => {
      // Route each off-spec stream to its best value: dehydrated > feed > landfill.
    ...
    const ev = L.evidence(
      ...
      [ { name: "recovery-route value ranking", weight: 0.18 },
        { name: "divert-vs-landfill economics", weight: 0.10 } ]);
    L.explain(ev);
  ...
  agri_line_scheduling(L) {
    ...
    const per = rows.map(r => {
      ...
      return { line: r.line_id, sku: r.sku_id, plan: feasible, committed: r.committed_cwt,
        shortfall, util, rawLimited: r.raw_avail_cwt < r.committed_cwt };
    });
    const short = per.filter(p => p.shortfall > 0);
    // Raw-limited lines can't meet commitment → lower confidence, route to human.
    ...
    ev.detailed.recommendedAction = short.length
      ? `Schedule feasible volume now; escalate ${short.map(s => s.line).join(", ")} shortfall to S&OP (source raw or re-cut commitments).`
      : `Release schedule at ${per.map(p => `${p.line} ${Math.round(p.util * 100)}%`).join(", ")} utilization.`;
...
Object.assign(USE_CASES, {
  agri_fresh_supply: {
    ...
    gen1Agents: ["Supply Balance Agent", "Shelf-Life / FEFO Agent", "Demand Sensing Agent", "Cold-Chain Inventory Agent",
      "Reefer OTIF Agent", "Input Hedge Agent", "Surplus Recovery Agent", "Line Scheduling Agent"],
    plan: [
...
// Why you would run each agent, in business terms: the trigger event, the
// objective, the business decision it drives, and the value lever. Surfaced on
// every run alongside the per-decision technical explainability, so a reader
// sees BOTH "why run this" (business) and "how each call was decided" (evidence).
...
Object.keys(AGENT_BUSINESS).forEach(k => { if (USE_CASES[k]) USE_CASES[k].business = AGENT_BUSINESS[k]; });

/* ---------------------------------------------------------------------------
 * Optional LIVE enrichment of the per-decision explainability. Every decision
 * already carries a deterministic ev.detailed (0 tokens). When a model is
 * available, this rewrites the prose fields (rootCause, reasoningSteps,
 * recommendedAction) into richer, decision-specific language and marks
 * source:"live". Honest: only the gated / lower-confidence decisions are
 * enriched (capped by `max`) to control cost; the rest stay template.
 * `chatJSON` is injected by the caller (server) so the engine stays dependency-free.
 */
async function enrichExplainability(evidenceLog, chatJSON, max = 5) {
  ...
  // Prioritise the decisions a reviewer most needs explained: held / gated / lowest confidence.
  const ranked = evidenceLog
    .map((ev, i) => ({ ev, i }))
    .filter(x => x.ev && x.ev.detailed)
    .sort((a, b) => (Number(a.ev.autonomous) - Number(b.ev.autonomous)) || (a.ev.confidence - b.ev.confidence));
  let enriched = 0;
  for (const { ev } of ranked.slice(0, Math.max(0, max))) {
    const d = ev.detailed;
    const sys = "You are an SAP/supply-chain decision analyst writing audit-grade explainability. " +
      "Given a governed decision and its evidence, reply ONLY as JSON " +
      '{"rootCause":string,"reasoningSteps":string[4],"recommendedAction":string}. ' +
      "Be concrete and specific to THIS decision; do not restate the numbers verbatim; no preamble.";
    const user = JSON.stringify({
      decision: d.decision, outcome: d.outcome, confidencePct: d.confidencePct,
      thresholdPct: d.gate.thresholdPct, primaryDriver: d.primaryDriver,
      contributingFactors: d.contributingFactors, signalIntegrity: ev.integrity || null,
    });
    let r; try { r = await chatJSON([{ role: "system", content: sys }, { role: "user", content: user }], { max_tokens: 320, tier: "medium" }); } catch (_) { r = null; }
    if (r && r.ok && r.data && (r.data.rootCause || r.data.reasoningSteps)) {
      ...
      d.source = "live"; ev.explanationSource = "live"; enriched++;
    }
  }
  return { enriched };
}
...
if (typeof module !== "undefined" && module.exports) {
  module.exports = { UNAI, SuperAgent: UNAI, USE_CASES, ONTOLOGY, OntologyMapper,
    ...
    getVectorStore, vectorStoreStatus, VECTOR_CATALOGUE, VECTOR_PROVIDERS,
    EmbeddedVectorStore, unaiEmbed, RAG_SEED, Layers, enrichExplainability,
    SemanticQueryCache, normalizeQuery, CONCEPT_SYNONYMS };
}
// browser back-compat: expose both names as globals
if (typeof window !== "undefined") { try { window.UNAI = UNAI; window.SuperAgent = UNAI;
  window.vectorStoreStatus = vectorStoreStatus; window.VECTOR_CATALOGUE = VECTOR_CATALOGUE; } catch (e) {} }
