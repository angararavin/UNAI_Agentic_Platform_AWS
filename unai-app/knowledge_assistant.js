...
class KnowledgeAssistant {
  ...
  async ask(query, user = { id: "anon", groups: [] }) {
    ...
    let body, grounded = false, provider = "modeled", real = null;
    if (cfg.ok) {
      const r = await answerGrounded(query, context, cfg).catch(() => null);
    ...
    return { type: "answer", grounded, provider, economics,
